...
- (void)completeTransaction:(SKPaymentTransaction *)transaction {
    ...
    NSLog(@"购买完成");
    NSLog(@"购买成功 %@  --- 订阅的 ID %@",transaction.transactionIdentifier,transaction.originalTransaction.transactionIdentifier);
...
- (void)failedTransaction:(SKPaymentTransaction *)transaction {
    NSLog(@"购买失败 %@",transaction.transactionIdentifier);
...
- (void)paymentQueue:(SKPaymentQueue *)queue updatedTransactions:(NSArray *)transactions
{
    ...
    //pay fiail 没查到订单  ORDER_ID_INVALID 订单重复
    for (SKPaymentTransaction *transaction in transactions)
    {
        //所有订单状态: 1-----1000000657170711
        //所有订单状态: 1-----1000000657170742
        NSLog(@"所有订单状态: %ld-----%@",(long)transaction.transactionState,transaction.transactionIdentifier);
...
- (void)buyProduct:(SKProduct *)productIdentifier  uid:(NSString *)uid onCompletion:(IAPbuyProductCompleteResponseBlock)completion {
